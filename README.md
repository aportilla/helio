...
**Bias model.** The catalog is treated as dramatically incomplete for every star, but v1 doesn't augment partially-observed stars — if a star has at least one catalog planet, the Architect skips it and the system contains only its catalog rows. A future overlay pass will add procgen extras around catalog anchors (e.g. outer planets that radial velocity surveys couldn't have detected).
...
`SystemHud` mirrors `MapHud`'s structure (own scene + ortho camera + composed widgets, `autoClear` off). Two pieces of chrome float against the top edge with the same `edgePad` inset on both sides: an `IconButton` back-arrow in the top-left and the system name title in the top-right. The name is display-only (pointer-transparent); the back button is the only interactive surface. No info card, no settings panel.

The scene's body is a flat 2D diagram, not a 3D space. `SystemDiagram` (in `src/scene/system-diagram.ts`) owns its own `OrthographicCamera` at 1 unit = 1 buffer pixel — the same convention as `Labels` and the HUD overlays. Layout has two zones stacked at fixed distances from the top edge, so the stars + bodies form a constant-height block hanging from the top and the empty space below (reserved for ships) grows or shrinks with the viewport:

- **Stars row.** Each cluster member is a `Mesh + PlaneGeometry` disc whose center sits *above* the buffer top by `STAR_OFFSCREEN_FRAC = 0.3` of the disc radius, so the GPU clips the offscreen portion and the visible sliver reads as "huge body, mostly hidden up there". Mesh (not `Points`) is load-bearing: `gl.POINTS` discards any sprite whose vertex falls outside the clip volume, but the triangle path rasterizes fine with vertices outside the viewport. Disc sizes preserve the within-class ratios from the catalog's `pxSize` field, multiplied by `DISC_SCALE = 9`. Lateral order puts the biggest star in the middle with smaller members flanking; the row narrows by scaling discs down (not by tightening the gap) once `STAR_HORIZ_GAP_FACTOR × largest-diameter` falls below `MIN_STAR_GAP`.
- **Body dome.** Planets sit on a peak-anchored arc below the stars. The peak is fixed at `PLANET_PEAK_FROM_TOP = 100` env-px below the top edge; the edges drop by an amount that lerps from `DOME_PEAK_MIN_PX` to `DOME_PEAK_MAX_PX` as viewport *area* grows from 400k to 2M env-px². Area (rather than width or height alone) drives the lerp because the arc reads as "proportional to how much real estate you have." Planets are placed in semi-major-axis order, innermost left to outermost right. Moons straddle their parent's rim at angles seeded per planet and distributed by largest-gap-fill with geometric per-pair margins so adjacent moons land tangent rather than touching, then split into back/front pools so the upper half is occluded by the parent and the lower half overlaps downward.

Disc sizing for bodies uses cube-root compression on `radiusEarth`. Planets: `cbrt(radiusEarth) × PLANET_DISC_BASE` (= 54), clamped to `[40, 120]` px — Mercury (0.38 R⊕) lands near the floor, Jupiter (11.2 R⊕) near the cap, ~3× ratio overall. The previous sqrt curve was producing 4-px specks next to 20-px gas giants (~5×) which read as "dominant disc vs. speck"; cbrt collapses the real ~30× radius range into a more legible 3×. Moons use the same cbrt curve clamped to `[10, 50]` px; the 50-px moon cap deliberately exceeds the 40-px planet floor because big moons cluster around big planets (Ganymede / Titan orbit gas giants), so a 50-px moon always sits next to a 100+ px parent in practice.

There's no orbit, no zoom, no depth attenuation; the only pointer interaction in system view is clicking the back button.
