...
| `lib/procgen.mjs` | Body Filler — bottom-up procgen. Walks empty cells in topological order: `radiusEarth` from a mass-radius relation, then the `worldClass` cascade (`avgSurfaceTempK`, `surfacePressureBar`), then `periodDays ↔ semiMajorAu` via Kepler's third law (bidirectional, so RV and transit discoveries both round-trip), then orbital flavor (eccentricity / inclination / axial tilt / orbital phase). Exports `radiusFromMass`, `worldClassFor`, `planetTypeFor` for the moon-backfill pass to reuse. Imported by `build-catalog.mjs`. |
