...
departure-banner.ts     DepartureBanner: the floating on-map overlay during a galaxy warp destination pick — a bottom-center HUD plate ("Select a destination" → "<dist> light years, <eta> turns") with inline CONFIRM (once locked) / CANCEL pills. Floats over the stars (own ortho pass, no layout reserve); StarmapScene owns it. Its sidebar twin is the lit convoy tiles in GalaxyContext
map-hud/
...
encounter-hud/
  ...
  active-pip.ts         ActivePip: the acting side's frontier pip lifted out of the row + shimmered along its own slant by adding/removing whole pixel rows at its ends (crawl-free; pre-baked per-shift variants, controller-animated) — the 'whose phase' cue
...
sidebar/
  ...
  galaxy-context.ts     GalaxyContext: the galaxy body — the game-VIEWS menu when nothing is selected (Galaxy live; Ships/Planets/Research dim placeholders) OR the selected system's SHIPS list, each ship a full-width selectable TILE (its `paintShipHull` module-sprite + name + a convoy CHECKBOX); checking any ship enters the nav destination pick as a CONVOY (they travel together) — the galaxy-only nav entry — and the ship list stays LIVE (check/uncheck to grow/shrink the convoy, unchecking the last exits). The checked set is the single source of the convoy, owned here (the CHECKBOX toggles a ship in/out for multi-select, a tile-body click selects just that one; cleared on confirm/cancel + system-change; pruned vs the live ready list) and reports repaint-needed from handleClick. A checked tile paints LIT. Footer: pan/zoom when idle; suppressed while any ship is checked (the on-map DepartureBanner owns confirm/cancel); View System / Deselect for a plain selected system.
