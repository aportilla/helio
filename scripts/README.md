...
| `check-sim-boundary.mjs` | Enforce the standalone-sim import wall. Fails if any file under `sim/src/` imports the app, or if any `src/` file **outside** `src/facilities/` imports the sim (static or dynamic `import()`). The repo has no ESLint, so this node script stands in for the plan's `no-restricted-imports` rule. Alias: `npm run check:boundaries`; also wired into `check.mjs`. |
| `check.mjs` | Validation umbrella for the iterative edit loop. Runs the validation gates — lint, build, type-check (app + tests), sim-boundary, procgen-audit (see `check.mjs` for the authoritative order) — in sequence and fails fast on the first non-zero exit. Catches dead CSV rows, schema regressions, type errors, sim-wall breaches, and out-of-envelope distribution shifts in one command. Alias: `npm run check`. |
