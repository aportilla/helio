...
| `lib/procgen-architect.mjs` | System Architect — top-down procgen. For each star with zero catalog planets, samples a full planetary system (planets + moons + rings + belts) from the priors. Exports `generateMoons` and `generateRing` for the catalog-planet backfill pass in `build-catalog.mjs`. |
