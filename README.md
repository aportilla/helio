...
- **Belts** consume a row slot the same way planets do (`BELT_SLOT_WIDTH` env-px) but render as a vertical scatter of angular polygon blobs centered on the slot's cx/cy and reaching `BELT_HEIGHT_FACTOR` × the largest planet disc above and below. Each chunk picks a silhouette from the belt's shape library (`POTATO_SHAPES` / `CRYSTAL_SHAPES`, selected by `shapesFor` — irregular convex polygons inscribed in the unit circle), a half-extent from `BELT_CHUNK_SIZES`, and a random rotation — seeded per-chunk off the belt's id so the silhouette stays stable across reloads. The whole belt pool is one indexed triangle Mesh drawn through `makeBlobMaterial` (flat-color triangle fill, per-vertex hover flag); chunk count scales log-uniform with belt mass between `BELT_CHUNKS_MIN` and `BELT_CHUNKS_MAX`. Belt color comes from the resource grid via `bodyIcyness` — chunks lerp from `BELT_RING_COLOR_ROCKY` (brown-tan; Main Belt) toward `BELT_RING_COLOR_ICY` (pale cyan; Kuiper Belt) along the volatiles-vs-rocky axis, and the shape library switches to faceted shards (`CRYSTAL_SHAPES`) once icyness crosses 0.5. One chunk primitive, one color/shape derivation — visual character emerges from the same resource grid that drives gameplay.
