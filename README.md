...
- **Surface mode** — when `isBandedAtmosphere(body)` returns false. Palette is `[world-class color, dominant resource 1, chromophore | dominant resource 2]`. If a chromophore is set, it replaces the second resource accent so cloud cover (H2O on biotic Earths) or dust haze (DUST on Mars-class deserts) paints visibly without overwhelming the surface speckle.
