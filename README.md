...
- **Moon backfill** (in `scripts/build-catalog.mjs`, calling the Architect's exported `generateMoons`) — for each catalog planet that arrived with no moons in the CSV, derive a transient `worldClass` + `planetType` from its mass and orbital context and synthesize a moon set. Closes the "observed exoplanets almost never carry moon coverage" gap. **Curated systems are exempt** via `CURATED_SYSTEM_HOSTS` (Sol today) — their CSV silence is "really none," not "we don't know"; extend the set as other systems get fully curated.
...
**Bias model.** The catalog is treated as dramatically incomplete for every star. The Architect still leaves partially-observed systems alone at the planet level — a future overlay will fill in absent siblings around catalog anchors (e.g. outer planets radial-velocity surveys couldn't have detected). The moon-backfill pass is the v1 exception: every catalog planet without CSV moons gets a synthesized set, because detection methods almost never surface satellites and an exoplanet that reads as moonless in-game would contradict the rest of the bias.
...
**Hover info card.** `SystemDiagram.pickAt(x, y)` is the picker for the system view. Priority follows render order so what's visually on top wins: front moons → planets → back moons → stars. The first disc whose `(x − cx)² + (y − cy)² < r²` test passes is returned, no smaller-radius tiebreaker. The picked body drives two side effects each `pointermove`: `SystemDiagram.setHovered(pick)` stamps a 1-px white rim onto that disc by flipping a per-vertex `aHovered` attribute (planets/moons) or a `uHovered` uniform (stars), with the fragment shader swapping the rim's color when the disc's `length(d) > vRadius - 1.0`; and `SystemHud.setHoveredBody(pick, x, y)` updates the `BodyInfoCard` tooltip — a `BasePanel`-based widget that rebuilds its canvas only when the picked body changes, then places itself with a small cursor offset and edge-flip when near a viewport edge. `pointerleave` on the canvas clears both. Skipped when the cursor is over interactive HUD chrome (the back button), so a tooltip can't appear under what the user is aiming at.
