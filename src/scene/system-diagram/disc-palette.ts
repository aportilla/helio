...
// ── Lava / molten-surface emission drives ──
// Self-luminous incandescence emerges from THREE physically distinct melt
// sources, each a continuous smoothstep — never a `worldClass === 'lava'`
// gate. The shader composes them into one molten sub-pass; these scalars
// fold them down to (moltenCoverage, emissionTempNorm):
...
const OCEAN_FALLBACK_COLOR: readonly [number, number, number] =
  [SOLVENT_COLOR_H2O.r, SOLVENT_COLOR_H2O.g, SOLVENT_COLOR_H2O.b];

function oceanColorFor(body: Body): readonly [number, number, number] {
...
export function buildDiscPalette(
  ...
  // ── LAVA / MOLTEN-SURFACE EMISSION — two continuous drives, no gate.
  // heatMelt: the surface approaches melt as avgSurfaceTempK crosses the
  // silicate solidus. ventMelt: silicate magma reaches an actively-repaved
  // surface even when the surface is cold (Io); the high surfaceAge window
  // keeps Earth (age≈0.7) at zero, and the `refractory` factor keeps the
  // emission off ice/volatile-rich bodies whose tidal heat drives cold
  // cryovolcanism rather than glowing rock. The emission temperature then
  // rides on ventMelt, so a less-refractory body erupts cooler material
  // that the shader's emitNorm > 0 (Draper) guard drops to no glow — the
  // suppression is continuous, not a class branch.
  let moltenCoverage = 0;
  let emissionTempNorm = 0;
  let lavaSulfurFrac = 0;
  if (!surfaceSuppressed) {
    ...
    emissionTempNorm = clamp01((emissionT - LAVA_EMIT_T_MIN) / (LAVA_EMIT_T_MAX - LAVA_EMIT_T_MIN));
