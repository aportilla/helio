...
2. **Cloud layer** (when `vCloudCoverage > 0`). Two geometries, chosen per body by `vCloudStructure`:
    ...
    - **Banded** (`vCloudStructure ≥ 0.5`) — sphere-projected latitude strips picking from `vCloudPalette` / `vCloudWeights` by per-band hash. Disc is rotated into a band-aligned frame using `vTilt` (= `bodyVisualTiltRad(body)`, shared with the ring layer so a ringed giant's bands and ring share one vantage), projected as a sphere viewed obliquely with `RING_MINOR_OVER_MAJOR` foreshortening (bands curve as latitude lines), divided into a radius-scaled count of non-uniform-width strips, and perturbed with a chunky horizontal warp so edges undulate like turbulent zonal flow. Per-disc band count = `clamp(vRadius × BAND_DENSITY, 6, MAX_BAND_COUNT)` so a Uranus-class disc gets ~30 bands and a Venus-class moon gets ~14. Each band takes a small ±`BAND_LIGHTNESS_JITTER` additive RGB shift after its palette pick — resolves to readable cream/tan/dark-tan strips even when the palette collapses to near-identical hues. Painted at alpha = `vCloudCoverage` (typically 1.0 for a full deck).

   For banded clouds the palette comes from `topGases(body)` — atm1/2/3 colors plus the cloud-condensate species via `CHROMOPHORE_COLOR` override — then collapsed toward the weight-proportional mean by `BAND_BLEND_TOWARD_MEAN` so bands share a dominant tone with small per-band variation. For patchy clouds the palette degenerates to a single condensate color (slot 0; `CLOUD_COLOR_FOR_GAS` lookup with `GAS_COLOR` fallback).

3. **Haze layer** (when `vHazeOpacity > 0`). A uniform per-fragment lerp `mix(col, vHazeColor, vHazeOpacity)` across the whole disc, modeling well-mixed aerosol scattering at planetary scale (no condensation latent heat → no structure). Titan's tholin orange, Venus's sulfate yellow, Mars's dust rust live here. Earth has `vHazeOpacity = 0` so this is a no-op.
...
4. **Rim** — outward halo extending 0–3 px into space + an inward fade across the visible limb. Outward width is bucketed off `log10(surfacePressureBar + 1)`; no-surface bodies (gas giants) get a fixed default. Inward width is `floor(vRadius × INWARD_RIM_FRACTION)` (column thickness scales with how much sphere the line-of-sight crosses, not with pressure). Band widths inside the inward fade grow `1, 2, 3, … px` from the limb inward following the sphere-projection foreshortening curve (`bandIdx = floor((sqrt(1 + 8d) − 1) / 2)`). Each band's opacity comes from the closed-form **stroke-stacking** `1 − (1 − α)^stackCount` — equivalent to painting `stackCount` concentric back-to-front strokes at base alpha α. A per-pixel hash dither on `distFromLimb` scatters band boundaries so the fade reads as organic haze rather than concentric stripes. The outward halo outputs `vec4(vHazeColor, rimA)` against the planet material's `transparent: true` so it alpha-blends with rings, moons, and bodies behind it.
   ...
   The single rim color is picked CPU-side by physical regime in `disc-palette.ts`: haze species color when a haze layer is present (Titan tholin, Venus sulfate); the top-of-`topGases` color on no-surface bodies (H2 cream for Jupiter/Saturn, CH4 cyan for ice giants and Hycean — same source the cloud bands draw from, so rim and bands agree); Rayleigh sky-blue on surface bodies with thick clear air; nothing on thin / airless bodies.

**Visual weight ≠ molar fraction.** Painting each gas proportionally to its atm fraction reads wrong because H2 and He are essentially transparent while CH4 absorbs strongly per molecule and NH3 condenses into visible aerosols. `topGases` weights each candidate by `frac × potency × boost`:

- `GAS_POTENCY[gas]` — per-molecule absorption strength. H2/He/N2/Ar sit at 0.1 (transparent); CO2/O2/CO at 1.0; H2O/NH3/H2SO4/SILICATE/DUST at 3.0 (cloud-former); CH4 at 6.0 (Uranus/Neptune blue); SO2 at 8.0 (Venus haze).
- `CLOUD_VISUAL_BOOST` — multiplier applied to the cloud-species candidate before mixing in. Lets a full-coverage cloud deck show up in the band palette alongside the bulk atm gases without overwhelming them; calibrated so Jupiter's NH3 lands at ~37% of the renormalized palette (~2 of 6 bands rendering as belt-brown).
...
- `GAS_COLOR[gas]` — the gas's clear-air hue. Used for atm1/2/3 slot lookups and as the fallback for the cloud / haze paths.
- `CHROMOPHORE_COLOR[gas]` — the same gas's condensed-phase product hue, used when the gas enters via the cloud or haze layer (not gas-phase atm). Different physics, different color: CH4 gas → cyan-blue (Uranus absorption); CH4 condensate → tholin orange (Titan haze). NH3 gas → cream; NH3 condensate → NH4SH brown (Jovian belt). The AtmGas vocabulary also includes aerosol-only species (`SILICATE`, `DUST`) that never appear as atm gases — they only ever enter via the cloud or haze paths.
...
**Procgen — regime-keyed cloud and haze.** The Filler runs `cloudHazeRegimeFor(body, S)` (pure physics dispatch — no class input) to bucket the body into one of nine regimes:

- `hot_gaseous` — gaseous + T > `hotGaseousTempK` → SILICATE clouds + SILICATE haze
- `hycean` — gaseous + low insolation + bulkWater present → H2O clouds, no haze
- `cold_gaseous` — gaseous + T < `coldGaseousTempK` → CH4 clouds, no haze (CH4 IS the visible Uranus/Neptune blue)
- `temperate_gaseous` — gaseous default → NH3 clouds (Jupiter-class), no haze (NH3 condensate enters the cloud palette via `CHROMOPHORE_COLOR`)
- `cold_terrestrial` — terrestrial + T < `coldTerrestrialTempK` → sparse CH4 cloud + thick CH4 tholin haze (Titan-class)
- `volcanic` — terrestrial + (T > `volcanicTempK` or P > `volcanicPressureBar`) → H2SO4 banded cloud + SO2 sulfate haze (Venus-class)
- `biotic_wet` — terrestrial + carbon-aqueous biosphere ≥ microbial → patchy H2O cloud, no haze (Earth-class with active biosphere)
- `wet_terrestrial` — terrestrial + waterFraction ≥ threshold → patchy H2O cloud, no haze
- `dust_terrestrial` — terrestrial default → sparse H2O cirrus + DUST haze (Mars-class)

Each regime defines a coverage / structure / opacity sample range; the Filler draws once per body. Curated Sol bodies (Mercury, Venus, Earth, Mars, Jupiter, Saturn, Uranus, Neptune, Titan, Io, etc.) set cloud and haze explicitly in `bodies.csv` and bypass the regime dispatch.
