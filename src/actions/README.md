...
- **Body loadouts + always-shown categories (shipped):** bodies now field a real two-category
  ...
  **category palette** (`Actor.categories`): ships and bodies alike always show **Attack + Support +
  Command** (`command` a reserved placeholder — no module grants it yet, so it renders greyed), each
  empty category greyed-but-shown rather than hidden. Effects remain no-op stubs.
