...
| `src/actions/` | System action menu — the general system-view interaction grammar (select an **actor** → category → command, target locked live in the field); a neutral core leaf whose actor commands are **derived** from each provider's grants (ship components / body facilities), not a central enum | [src/actions/README.md](src/actions/README.md) |
