...
**Why JSON, not TS.** No new dependencies (the build script is plain Node, no TS loader needed), Vite imports JSON natively, the artifact is debuggable in any editor. Nothing reads the generated file by hand, so autocomplete in it buys nothing.
...
| `audit-variety.mjs` | Game-feel census of iconic / exotic body archetypes — how often each recognizable SF trope shows up and what the rarest finds are, with named exemplars. Companion to `audit-procgen` (game-feel framing, not calibration). Alias: `npm run audit:variety`. |
| `check-disk-physics.mjs` | Disk-physics anchor regression gate: prints the frost-line / surface-density / isolation-mass anchors driven by the priors and exits non-zero if a gate-severity anchor falls out of band. No catalog dependency — fast iteration on the disk-physics constants. Alias: `npm run check:disk-physics`. |
| `check-sim-boundary.mjs` | Enforce the standalone-sim import wall. Fails if any file under `sim/src/` imports the app, or if any `src/` file **outside** `src/facilities/` imports the sim (static or dynamic `import()`). Resolves specifiers to absolute paths and checks directory containment — more robust for this path-based zone than oxlint's `import` plugin, so this node script (not the linter) owns the wall. Alias: `npm run check:boundaries`; also wired into `check.mjs`. |
| `check.mjs` | Validation umbrella for the iterative edit loop. Runs the validation gates — CSV lint, oxlint, build, type-check (app + tests), sim-boundary, procgen-audit (see `check.mjs` for the authoritative order) — in sequence and fails fast on the first non-zero exit. Catches dead CSV rows, lint violations, schema regressions, type errors, sim-wall breaches, and out-of-envelope distribution shifts in one command. Alias: `npm run check`. |
...
App-domain tooling (`inspect-economy.ts` — the headless economy inspector — and `screenshot.mjs` — the app screenshotter) also lives under `scripts/`, but it's documented in [docs/dev-tooling.md](../docs/dev-tooling.md) rather than this catalog-tooling table.
...
```bash
npm run check               # the validation umbrella — see scripts/check.mjs for the gates + order
```
...
- **Hand-written `.d.mts` declarations**: `lib/prng.mjs`, `lib/body-traits.mjs`, and `lib/gas-potency.mjs` ship a sibling `.d.mts` so the typed app bundle re-imports the same runtime JS under strict typing. Nothing auto-checks the two stay in sync — when you add or rename an export in one of those `.mjs` files, update its `.d.mts` by hand (a missing entry just goes invisible to the typed side; `npm run typecheck` flags a renamed/removed one only where it's consumed).
