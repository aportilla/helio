...
- **Cap-boundary per-cell jitter** (done). The cap latitude threshold gets a per-worley-cell hash perturbation so the boundary follows the cell grid's already-jittered shapes instead of reading as a clean foreshortened ellipse. Cells well inside / well outside the cap are unaffected; only cells straddling the boundary flip in or out. Amplitude `clamp(iceFrac × 0.6, 0.01, 0.08)` so Mars's tiny cap (iceFrac 0.02 → ±0.012) keeps a visible-but-not-noisy edge while Earth's larger cap (iceFrac 0.10 → ±0.06) gets a more obvious ragged coastline. Salts 233/239. No new attributes; one extra hash in the cap branch.
- **Cap regime partitioned from global regime** (done). The per-fragment ice flag stopped being a continuous `mix(capPriority, globalPriority, globalness)` and became two discrete patterns split at `CAP_GLOBALNESS_MAX = 0.8`. Warm / transitional bodies (Earth, Mars; `globalness < 0.8`) run the cap-pattern only — `|latSinDisc| > 1 − iceFrac` paints pure `ICE_COLOR`, bypassing the `surfaceAge` blend. Cold bodies (Europa, Callisto, Triton; `globalness ≥ 0.8`) run the global-pattern only — cell-hash > `1 − effectiveIceFrac` paints into the existing `surfaceAge`-blended stack so young icy moons stay bright and old icy moons keep their regolith-buried look. Two reasons for the change: (a) the previous mix diluted Mars's cap signal (74% global at iceFrac 0.02 → ~0 visible cap fragments) so the polar cap vanished entirely; (b) the cap is a surface-deposit feature, distinct from bulk cryosphere — Mars's caps are seasonal water+CO2 ice on the surface, not regolith-buried ice. Companion fix: cap and biome latitude tests use a new `latSinDisc` computed from the **un-inset** normal so the visible disc rim reaches the true sphere pole (the 1.5a `SPHERE_VISIBLE_FRAC = 0.85` inset stays in effect for the worley cell pass that needs bounded foreshortening, but the cap test was clipping small-iceFrac caps under the inset's visible-latitude ceiling of ~0.94). Earth / Mars polar caps visible again; Europa / Callisto / Ganymede unchanged.
...
Pole singularity: `lon` is ill-defined at `latSin = ±1` (the visible pole). The inset bounds the max `latSinS` (the worley-pass latitude) to `FRAC × POLE_COS + sqrt(1 − FRAC²) × POLE_SIN`, so the singularity is never reached by the cell coords; the cap test and biome lat taper use a parallel `latSinDisc` computed from the **un-inset** normal so the visible disc rim still reaches the true pole — otherwise small-iceFraction caps (Earth ~0.05, Mars ~0.03) fall under the inset's visible-latitude ceiling and clip entirely. Airless capless bodies have no visible singularity by construction.
...
## Phase 1.6 — Ice as a contextual surface state (DONE)
...
**Per-fragment ice flag (`icyHere`) — geometry decider.** Two physically distinct patterns, partitioned (not mixed) by `vGlobalness` at the `CAP_GLOBALNESS_MAX = 0.8` boundary. Cap pattern is a surface-deposit feature: pure `ICE_COLOR` at high latitude, bypasses the `surfaceAge` blend, fires only on warm / transitional bodies. Global pattern is bulk cryosphere: scattered cells across all latitudes, `surfaceAge`-blended (young → ice on top, old → regolith on top), fires only on cold bodies. Both feed `icyHere` for the crater / linea / cloud-suppression branches downstream, but only one writes the surface color per body.

Cap-pattern uses `latSinDisc` — derived from the **un-inset** disc normal — so the visible disc rim reaches the true sphere pole. The 1.5a `SPHERE_VISIBLE_FRAC` inset stays in effect for the worley cell pass (which wants bounded foreshortening), but applying the inset to the cap test clipped small-iceFrac caps under a visible-latitude ceiling of ~0.94 — Mars at iceFrac 0.02 needs latitude up to 0.98 to fire.
...
The cap-pattern threshold takes a per-cell hash jitter (`capJitter`) so the boundary follows the worley grid's organic edges. Amplitude scales with `iceFrac` so small caps wobble less than large caps — see "Cap-boundary per-cell jitter" landing above for tuning rationale.

`vGlobalness` is CPU-precomputed via `smoothstep(ICE_TEMP_CAP_K = 270, ICE_TEMP_GLOBAL_K = 180, avgSurfaceTempK)` in `disc-palette.ts` (descending lerp so cold T → globalness 1, warm T → globalness 0), packed into the spare slot of `aAtmoStrokes`. Earth (288 K) → globalness 0 → cap pattern at iceFrac 0.10; Mars (210 K) → globalness 0.74 → cap pattern at iceFrac 0.02; Europa (102 K) → globalness 1 + `frozenBoost` 1 → `effectiveIceFrac` 1 → every cell `globalIcyHere=true`, full ice shell with linea carrying the non-ice signal. `CAP_GLOBALNESS_MAX = 0.8` lines up with the lower edge of `frozenBoost`'s smoothstep — a body either stays fully in the cap regime (frozenBoost 0) or transitions into the global regime (frozenBoost > 0), never both. The 0.8 crossover lands at T ≈ 206 K — comfortably below Mars, comfortably above any of the catalog's outer-system icies.
