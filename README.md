...
1. **Surface layer** (only when `hasSurface` is true — terrestrial bracket classes). Sphere-projected worley/voronoi cell texture; cells live in `(lon, lat)` on the band-aligned sphere so the disc reads as a globe. Palette is resource-driven via `dominantResources(body, 3)` — Mercury's iron-grey + rare-earth rose + silicate rust mirrors its mining yields directly. World-class color only re-enters as a flat-fill fallback when a body has no resource signal. Gas / ice giants and hycean / helium classes skip the surface block entirely — their cloud layer is the canvas, with `vCloudPalette0` as the bulk-gas fallback color underneath.

  Inside the surface block, three sub-passes compose with stack order keyed off `surfaceAge`:

    - `resourceSurface` — the body's bulk character. A coarse `CONTINENT_GROUP × CONTINENT_GROUP` continent-cell hash flags "water territory" against `waterFraction`; warm bodies (`vGlobalness < 0.5`) paint water cells as flat `OCEAN_COLOR`, cold bodies don't — on a frozen body the water IS the ice. Non-water cells run the region pass: a `REGION_PATCH_FACTOR` super-cell hash discretizes into one of `REGION_BUCKET_COUNT` non-empty subsets of `{palette0, palette1, palette2}`, masking the body's natural weights so each region paints from a different combination of its resources (regional dichotomy rather than uniform blend). Biome stipple (`biomePaintFor(body)`) flips individual land pixels to a pigment color derived from `biosphereArchetype × hostStar.cls`, coverage keyed to `biosphereTier`.
    - `iceLayer` — flat `ICE_COLOR`. Per-fragment binary `icyHere` decided by mixing a cap-latitude priority and a per-cell hash by `vGlobalness` (smoothstep on `avgSurfaceTempK` between `ICE_TEMP_GLOBAL_K` and `ICE_TEMP_CAP_K`). Cap mode peaks at the poles via `|latSin|`; global mode scatters across all latitudes. Threshold is `icePriority > 1 − effectiveIceFrac` with `effectiveIceFrac = mix(vIceFrac, 1.0, smoothstep(0.8, 1.0, vGlobalness))` — on bodies cold enough that liquid surface water is thermodynamically impossible the ice shell covers the entire surface.
    ...
    - **Craters** — discrete point features whose existence per crater-seed cell scales with `(1 − surfaceAge)²`. Crater color composes by stack order: young → reveals the body's other resources beneath the ice; old → reveals ice where ice exists, subsurface where it doesn't. Per-crater region (not per-fragment) drives the subsurface mask so a crater straddling a region boundary paints uniformly.
...
2. **Cloud layer** (when `vCloudCoverage > 0`). Two geometries, chosen per body by `vCloudStructure`:

    - **Patchy** (`vCloudStructure < 0.5`) — anisotropic worley cells in the equator-aligned frame (`CLOUD_LON_PX / CLOUD_LAT_PX` ≈ 2.4 : 1 east-west stretch for wind-swept silhouettes). Per-cell hash < `vCloudCoverage` paints the condensate color (`vCloudPalette0` — H2O white for Earth, CH4 frost for Titan, dust rust for a Mars storm). Models cellular convection on terrestrial-pressure atmospheres.
    - **Banded** (`vCloudStructure ≥ 0.5`) — two anisotropic worley layers in the same `(lon, lat)` sphere-projected frame the surface and patchy paths use. Cells are stretched east-west (`BAND*_LON_PX / BAND*_LAT_PX` ≈ 8:1) so the natural Voronoi tessellation forms strips along the rotation axis. Per-cell color is hashed off the cell's *latitude component only* — cells at the same latitude track resolve to the same palette pick, so bands read as parallel to the equator. Worley's natural jitter wobbles the longitude boundaries between adjacent bands, no separate warp pass needed. The primary layer covers the disc; a finer detail layer overpaints cells whose existence hash falls under `DETAIL_COVERAGE`. The disc is rotated by `vTilt` (= `bodyVisualTiltRad(body)`, shared with the ring layer so a ringed giant's bands and ring share one vantage) and projected as a sphere with `RING_MINOR_OVER_MAJOR` foreshortening (bands curve as latitude lines). Each cell takes a small ±`BAND_LIGHTNESS_JITTER` additive RGB shift after its palette pick. Painted at alpha = `vCloudCoverage`.

   For banded clouds the palette comes from `cloudBandPalette(body)` — four slots, picked per worley cell by frequency rather than blended into a common tone. Two physical mechanisms feed the palette differently: **atm gases** (column absorption, uniform across the disc) contribute to the base blend only — they tint the whole disc but don't form spatial accents. **Cloud + haze species** (photic-depth chemistry, spatially varied) drive the accent slots AND fold into the base blend at reduced weight. Slot 0 is the **perceptual base blend** = weighted average across all three (atm species at full `frac × potency`, cloud species at `coverage × potency × CLOUD_VISUAL_BOOST`, haze species at `opacity × potency × HAZE_VISUAL_BOOST`). Slots 1-3 are the top three **cloud + haze species** as accents, each at full pigment saturation. Slot 0 takes `BASE_BLEND_WEIGHT` of the picker's weight on surface bodies; no-surface bodies (gas/ice giants) replace that with a temperature-derived per-body weight so colder bodies render more uniformly (Jupiter at 165 K gets ~0.50 → strong zone/belt contrast; Saturn at 134 K → ~0.78, heavily muted; Uranus / Neptune at ~75 K → ~0.85, almost featureless). The same temperature signal also drives an **accent desaturation**: as slot 0's share grows above the default, each accent slot is mixed toward the base blend by `accentMute = clamp((slot0 - BASE_BLEND_WEIGHT) × 2, 0, 1)`. Models stratospheric photochemical haze physically paling the chromophore chemistry visible through it — Jupiter's belts stay punchy because its cloud deck is largely clear, Saturn's NH4SH chromophore mutes because we're seeing it filtered through the hazy stratosphere above. Slots 1-3 share the remaining `(1 - slot0)` weight, renormalized. Result: Jupiter's H2/He cream column tints the base, NH3 ice deck warms it, and NH4SH chromophore punches through as brown belt accents — no Uranus-cream-stripe artifacts because column-absorption species can't appear as spatial accents. For patchy clouds the palette degenerates to a single condensate color (slot 0; `CONDENSATE_COLOR` lookup with `GAS_COLOR` fallback).

3. **Haze layer** (when `vHazeOpacity > 0` AND `vHasSurface > 0.5`). A uniform per-fragment lerp `mix(col, vHazeColor, vHazeOpacity)` across the whole disc, modeling a well-mixed photochemistry aerosol blanket — Titan tholin (~0.85), Venus sulfate (~0.7). The surface gate is load-bearing: on gas/ice giants the `hazeGas` species is altitude-localized chromophore chemistry (NH4SH belts on Jupiter, etc.), not a uniform overcoat. It feeds the cloud-band palette as an accent instead; running the uniform overlay on top would crush the band structure into a single chromophore tone.

4. **Dust layer** (when `vDustiness > 0` AND `vHasSurface > 0.5`). A second uniform overlay `mix(col, vDustColor, vDustiness)` running independently of the photochemistry haze above. Models lifted mineral dust — Mars rust (~0.2). Dust is a separate channel from `hazeGas` because the four photochemistry-product species (THOLIN / NH4SH / H2SO4 / SILICATE) share one mutually-exclusive slot per body, but mineral dust can coexist with any of them (a hot dusty terrestrial could carry both sulfate chemistry and lifted dust). `vDustColor` is body-specific — drawn from `dominantResources` so iron-grey on a metal-dominant world, rust on Mars's ferric-oxide surface, tan on silicate-rich. Dust IS surface material in suspension; the color reflects the actual mineralogy.

5. **Rim** — outward halo extending 0–3 px into space + an inward fade across the visible limb. Outward width comes from an Earth-normalized scale-height proxy `scaleHeightFactor = (T/288) / (M/R²) × µRatio`, where `µRatio = µ_air / µ_atm` is class-keyed (1 for surface bodies; ~13 for H₂/He no-surface classes; ~7 for helium). **Surface bodies** bucket off `log10(P+1) × scaleHeightFactor` — Titan (low g + bearable pressure) outpuffs Earth despite ~⅔ Earth's surface pressure; Venus caps out on its raw column thickness even after the gravity penalty. Surface bodies that fall through the pressure tiers but still produced a non-zero merged rim color (Mars: thin CO₂ trickle below the scattering gate, but dust storms + thin cirrus dominate the visible signal from orbit) get a `RIM_PRESENCE_FLOOR_PX = 1` floor so they read as "has air" rather than airless like Mercury / Luna; the 2 / 3 px tiers stay reserved for significant atmospheric depth. **No-surface bodies** (gas giants / ice giants / hycean / helium / gas dwarfs) bucket off `scaleHeightFactor` directly with a base 1 px (no surface pressure to anchor — at the limb you're looking through a deep continuous column whose pressure is conventional). Sol gas-giant anchors: Jupiter / Neptune 2 px (compressed by gravity), Saturn / Uranus 3 px (puffier per their real H/R ratio). A hot Jupiter saturates at 3; a cold high-grav procgen ice giant can drop to 1. Inward width is `floor(vRadius × INWARD_RIM_FRACTION)` (column thickness scales with how much sphere the line-of-sight crosses, not with pressure). Band widths inside the inward fade grow `1, 2, 3, … px` from the limb inward following the sphere-projection foreshortening curve (`bandIdx = floor((sqrt(1 + 8d) − 1) / 2)`). Each band's opacity comes from the closed-form **stroke-stacking** `1 − (1 − α)^stackCount` — equivalent to painting `stackCount` concentric back-to-front strokes at base alpha α. A per-pixel hash dither on `distFromLimb` scatters band boundaries so the fade reads as organic haze rather than concentric stripes. The outward halo outputs `vec4(vRimColor, rimA)` against the planet material's `transparent: true` so it alpha-blends with rings, moons, and bodies behind it. `LIMB_BRIGHTEN` (forward-scatter brightening — a uniform lerp of the rim color toward white) is held at 0 so the rim paints the pure data-merged color; the constant + comment stay in the shader as a tunable knob if dark hazes start vanishing against dark interiors, but the honest move there is to make it per-body off `scaleHeightFactor` rather than turning it back on globally.

   Rim color is a **weighted-average merger** across every visible atmospheric contributor (in `disc-palette.ts`). No regime branching — every channel with weight > 0 participates, weights sum, and the result normalizes:

   - **cloud** at slot 0 of the cloud palette (the perceptual base blend for banded clouds, the single condensate color for patchy) → weight = `cloudCoverage`. Already chromophore-muted via `cloudBandPalette`'s base-blend weighting, so safe on no-surface bodies.
   - **photochemistry haze** species color (THOLIN / NH4SH / H2SO4 / SILICATE) → weight = `hazeOpacity`. SKIPPED on no-surface bodies because those species are altitude-localized cloud-deck chromophores that don't reach the glancing-angle limb.
   - **clear-air scattering** — Rayleigh blend via `scatteringRimColor` on surface bodies (weight = `log10(P+1)` so Earth contributes much more than Mars's wisp, gated below `RIM_MIN_PRESSURE_BAR`); forward-scatter gas-column blend via `atmColumnColor` on no-surface bodies (weight = 1, since the gas column IS the rim physics there — H2/He cream for Jupiter, CH4-tinted cyan for Uranus).
   - **lifted dust** — body-resource-derived color via `dustColorFor` → weight = `dustiness`. Surface bodies only.

   The merged color drives both the outward halo and the inward limb fade. On Titan the merger lands tholin-dominated (haze opacity 0.85 outweighs the cloud and scatter contributions); on Earth it lands cyan-Rayleigh with a cloud-white assist; on Mars it lands dust-rust dominated; on Jupiter it lands H2/He-column with a cloud-deck assist. One code path, no regime cliff.

**Visual weight ≠ molar fraction.** Painting each gas proportionally to its atm fraction reads wrong because H2 and He are essentially transparent while CH4 absorbs strongly per molecule and NH3 condenses into visible aerosols. `cloudBandPalette` weights each contributor by `frac × potency × boost`:

- `GAS_POTENCY[gas]` — per-molecule absorption / aerosol strength. H2/He/N2/Ar sit at 0.1 (transparent); CO2/O2/CO at 1.0; H2O/NH3/H2SO4/SILICATE/THOLIN/NH4SH at 3.0 (cloud-former / aerosol product); CH4 at 6.0 (Uranus/Neptune blue); SO2 at 8.0 (Venus haze). DUST isn't in this table — it's tracked via the body's `dustiness` channel rather than as an `AtmGas`.
- `CLOUD_VISUAL_BOOST` and `HAZE_VISUAL_BOOST` — both 0.03. Equal weights now that the chromophore is a band accent rather than a uniform overlay; the asymmetry that used to favor haze (2× cloud) no longer earns anything. Together they damp the cloud / haze contributions enough that the bulk atm gases can dominate the base blend on hydrogen-helium worlds.
- `GAS_VISIBILITY_FILTER[worldClass]` — gases physically present but buried under a cloud deck. Gas-giant CH4 (under the NH3 ice layer) is dropped on `gas_giant`/`gas_dwarf` so Jupiter doesn't paint blue from CH4 invisible from above. Ice giants intentionally don't filter — CH4 IS what you see on Uranus/Neptune.
...
The renderer paints exactly what procgen emits: chemistry (e.g. CH4 + N2 + UV → THOLIN, NH3 + H2S → NH4SH) happens in `procgen.mjs` and lands in the body's `hazeGas` field as the actual product species. No table does silent chemical substitution. THOLIN, NH4SH, and SILICATE live in the `AtmGas` vocabulary as aerosol-only species (never emitted as atm). Mineral dust lives outside `AtmGas` entirely — it's a separate `dustiness` channel because dust isn't a chemistry product (it's lifted surface material) and isn't mutually exclusive with the photochemistry species.
...
**Procgen — cloud regime + physics-derived haze.** Cloud and haze come from independent procgen passes that share the body's physics inputs but no longer share a regime table.

*Cloud* emission goes through a 9-bucket regime dispatch. The Filler runs `cloudHazeRegimeFor(body, S)` (pure physics — no class input) to pick one of:

- `hot_gaseous` — gaseous + T > `hotGaseousTempK` → SILICATE clouds
- `hycean` — gaseous + low insolation + bulkWater present → H2O clouds
- `cold_gaseous` — gaseous + T < `coldGaseousTempK` → CH4 clouds (CH4 IS the visible Uranus/Neptune blue)
- `temperate_gaseous` — gaseous default → NH3 clouds (Jupiter-class)
- `cold_terrestrial` — terrestrial + T < `coldTerrestrialTempK` → sparse CH4 cloud (Titan-class)
- `volcanic` — terrestrial + (T > `volcanicTempK` or P > `volcanicPressureBar`) → H2SO4 banded cloud (Venus-class)
- `biotic_wet` — terrestrial + carbon-aqueous biosphere ≥ microbial → patchy H2O cloud (Earth-class with active biosphere)
- `wet_terrestrial` — terrestrial + waterFraction ≥ threshold → patchy H2O cloud
- `dust_terrestrial` — terrestrial default → sparse H2O cirrus (Mars-class)

Each regime defines a `cloudGas` + coverage + structure (patchy vs banded) sample range; the Filler draws once per body. The `cloudHazeRegimeFor` name reflects history — it used to drive haze too, but haze is now species-by-species.

*Haze* (`hazeGas` + `hazeOpacity`) is derived per species. `hazeFor(body)` walks candidates THOLIN, NH4SH, H2SO4, SILICATE. Each runs through a `hazeContribution(gas, body)` gate that fires on the body's actual physics (temperature, pressure, atm precursors, world class). Total opacity is summed and capped at 1.0; the highest-weighted contributor wins the `hazeGas` slot. The gates encode the chemistry directly:

- **THOLIN** — non-gaseous, T < ~130 K, requires CH4 *and* N2 in atm (Titan; methane alone on a cold ice-giant doesn't tholinize without the N2)
- **NH4SH** — gaseous, T in ~150–200 K window, requires NH3 cloud (Jovian belt brown; ice giants are too cold for the condensation)
- **H2SO4** — hot terrestrial + thick CO2 atm (Venus sulfate)
- **SILICATE** — hot gaseous (hot Jupiters / sub-Neptunes — refractive Mg-Si-O clouds)

*Dust* (`dustiness`) is the parallel channel for lifted mineral aerosol. Computed per-body by `dustinessFor(body)` — gates on terrestrial (not gaseous), some atmosphere to suspend in (P > 0), dry surface (water suppresses lift), thin enough air (P ≤ 1 bar so dust stays airborne), moderate temperature (150–300 K window). Mars hits the calibration target at ~0.2. Lives outside the `hazeGas` slot because dust can coexist with any photochemistry species — a hot dusty terrestrial could carry both sulfate haze chemistry and lifted dust simultaneously.

Procgen owns the chemistry: each branch outputs an explicit reaction product (THOLIN, NH4SH) so the renderer paints what the data says without chromophore inference. Curated Sol bodies (Mercury, Venus, Earth, Mars, Jupiter, Saturn, Uranus, Neptune, Titan, Io, etc.) set cloud and haze explicitly in `bodies.csv` and bypass both passes; `dustiness` is left empty even on curated rows so the gate-derived value applies uniformly (Mars's curated haze used to be `DUST,0.15`; under the new model the `dustinessFor` gates re-derive ~0.2 from the same physics).
