...
| `src/ships/` | Neutral ship-class registry (one `ShipClassDef` per type, each with a default component loadout) + the `components/` module registry — consumed by the sidebar build flow, the fleet render layer, and the action-menu derivation | [src/ships/README.md](src/ships/README.md) |
