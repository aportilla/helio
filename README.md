...
3. **Haze layer** (when `vHazeOpacity > 0` AND `vHasSurface > 0.5`). A uniform per-fragment lerp `mix(col, vHazeColor, vHazeOpacity)` across the whole disc, modeling a well-mixed aerosol blanket — Titan tholin (~0.85), Venus sulfate (~0.7), Mars dust (~0.15). The surface gate is load-bearing: on gas/ice giants the `hazeGas` species is altitude-localized chromophore chemistry (NH4SH belts on Jupiter, etc.), not a uniform overcoat. It feeds the cloud-band palette as an accent instead; running the uniform overlay on top would crush the band structure into a single chromophore tone.

4. **Rim** — outward halo extending 0–3 px into space + an inward fade across the visible limb. Outward width is bucketed off `log10(surfacePressureBar + 1)`; no-surface bodies (gas giants) get a fixed default. Inward width is `floor(vRadius × INWARD_RIM_FRACTION)` (column thickness scales with how much sphere the line-of-sight crosses, not with pressure). Band widths inside the inward fade grow `1, 2, 3, … px` from the limb inward following the sphere-projection foreshortening curve (`bandIdx = floor((sqrt(1 + 8d) − 1) / 2)`). Each band's opacity comes from the closed-form **stroke-stacking** `1 − (1 − α)^stackCount` — equivalent to painting `stackCount` concentric back-to-front strokes at base alpha α. A per-pixel hash dither on `distFromLimb` scatters band boundaries so the fade reads as organic haze rather than concentric stripes. The outward halo outputs `vec4(vHazeColor, rimA)` against the planet material's `transparent: true` so it alpha-blends with rings, moons, and bodies behind it.

   The single rim color is picked CPU-side by physical regime in `disc-palette.ts`. Three branches, checked in this order:

   1. **No-surface body** (gas/ice giant, hycean, helium) → `atmColumnColor(body)`. The limb cuts through a long glancing-angle gas column where forward scattering brightens the visible signal; the chromophore `hazeGas` species is altitude-localized and doesn't reach the limb, so it's deliberately excluded from this branch. H2/He cream for Jupiter/Saturn; CH4-tinted pale cyan for Uranus/Neptune.
   2. **Surface body with haze blanket** (`hazeGas !== null && hazeOpacity > 0`) → `hazeColor(hazeGas, body)`. Titan tholin orange, Venus sulfate yellow, Mars dust rust — the aerosol genuinely is a uniform overcoat the limb looks through.
   3. **Surface body with thick clear air** (`surfacePressureBar ≥ RIM_MIN_PRESSURE_BAR`) → `scatteringRimColor(body)`. Per-gas Rayleigh blend across atm1/2/3 by `frac × SCATTERING_POTENCY` — pale cyan-blue for N2/O2, deeper cyan for CH4-rich, yellow for SO2-tinted.

   Otherwise, no rim. The same color drives the outward halo, the inward limb fade, and (on surface bodies only) the uniform haze overlay — all three converge on one regime-picked tone.
...
- `GAS_POTENCY[gas]` — per-molecule absorption / aerosol strength. H2/He/N2/Ar sit at 0.1 (transparent); CO2/O2/CO at 1.0; H2O/NH3/H2SO4/SILICATE/DUST/THOLIN/NH4SH at 3.0 (cloud-former / aerosol product); CH4 at 6.0 (Uranus/Neptune blue); SO2 at 8.0 (Venus haze).
...
- `GAS_COLOR[gas]` — the species' visible hue when it appears as a gas-phase atm species OR as an aerosol haze. Carries CH4's absorption cyan, H2/He cream, plus the aerosol products (H2SO4 sulfate, SILICATE grey, DUST rust, THOLIN orange, NH4SH brown).
...
- **DUST** — dry-surface lift on terrestrials with moderate T and thin atm (Mars-class)

Procgen owns the chemistry: each branch outputs an explicit reaction product (THOLIN, NH4SH) so the renderer paints what the data says without chromophore inference. Curated Sol bodies (Mercury, Venus, Earth, Mars, Jupiter, Saturn, Uranus, Neptune, Titan, Io, etc.) set cloud and haze explicitly in `bodies.csv` and bypass both passes.
