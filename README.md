...
Visibility is per-drop, not via `group.visible`: the master HUD toggle sets `masterVisible`, and the current selection sets `selectedPrimary` (resolved from any cluster member). A drop renders when `masterVisible || drop.primary === selectedPrimary` — so toggling droplines off still preserves the selected star's pin as a depth cue for whatever the user has the info card open on.
