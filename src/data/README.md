...
Real radii in the catalog span a couple-hundred-fold range (Sirius B → Procyon A), so a linear mapping would make WDs invisible and A-class dwarfs dominate. Cube-root compression collapses that wide radius range into a narrow pixel range, so white dwarfs stay visible at the small end while A-dwarfs don't run away at the large end — keeping the class spacing legible without clipping. A `log10` mapping was tried first but over-compressed — it bunched M dwarfs and A dwarfs so close together that the brightest class barely stood out. Cube-root recovers roughly the same class spacing as the old per-class `CLASS_SIZE` table while keeping within-class variation: Wolf 359 (0.144 R☉) and Lalande 21185 (0.392 R☉) render as visibly different M dwarfs, which a class-keyed lookup couldn't express.
