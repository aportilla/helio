...
| `check.mjs` | Validation umbrella for the iterative edit loop. Runs `lint-star-csv` → `build:catalog --strict` → `tsc --noEmit` → `check-sim-boundary` → `audit-procgen` in sequence and fails fast on the first non-zero exit. Catches dead CSV rows, schema regressions, type errors, sim-wall breaches, and out-of-envelope distribution shifts in one command. Alias: `npm run check`. |
...
```bash
npm run check               # lint-star-csv + build:catalog --strict + tsc --noEmit + check-sim-boundary + audit-procgen
```
