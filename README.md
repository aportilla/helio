...
src/
  ...
  scene/                    Three.js — no DOM coupling beyond the canvas
    ...
    system-diagram/         SystemDiagram coordinator + per-render-kind layers
      ...
      disc-palette.ts       buildDiscPalette: per-body palette + mode + seed inputs for makePlanetMaterial
...
- **Surface mode** — when `isBandedAtmosphere(body)` returns false. Palette is `[world-class color, dominant resource 1, chromophore | dominant resource 2]`. If a chromophore is set, it replaces the second resource accent so cloud cover (H2O on biotic Earths) or dust haze (DUST on Mars-class deserts) paints visibly without overwhelming the ground texture. Surface texture is a worley/voronoi cell pass (`SURFACE_PATCH_PX` controls cell pitch) — the disc is divided into per-body jittered cells and each pixel takes its nearest cell's palette pick, so ground cover reads as organic lumpy patches rather than per-pixel salt-and-pepper noise.
