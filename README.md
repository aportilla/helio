...
- **Filler** (`scripts/lib/procgen.mjs`) — bottom-up derivation of remaining fields per body. Pipeline: `radiusEarth` from a piecewise mass-radius relation (Otegi-style), then the `worldClass` cascade (class from radius + insolation, with mass gates on the terrestrial branches — sub-Mars-mass would-be-oceans flip to `ice` via `OCEAN_MIN_MASS_EARTH` because they can't sustain atmospheric pressure for liquid surface water), `avgSurfaceTempK` via Stefan-Boltzmann + greenhouse offset, `surfacePressureBar` from world_class + escape-velocity proxy (with a Titan-class retention roll for mass-eligible `ice` worlds: `ICE_THICK_ATM_PROBABILITY` × `MIN_MASS_EARTH` flips ~15% of large icies from airless to 0.5–3 bar via `ICE_THICK_ATM_PRESSURE_BAR`). Then `periodDays ↔ semiMajorAu` bidirectionally via Kepler's third law (so RV-discovery rows and transit-discovery rows both round-trip), orbital flavor (`eccentricity`, `inclinationDeg`, `axialTiltDeg`, `orbitalPhaseDeg`) as seeded draws from the same priors the Architect uses, biosphere (archetype × tier, gated by world class + insolation), atmosphere composition (top-3 gases sampled per `ATMOSPHERE_GASES_BY_CLASS`, with an O₂ biotic-lift multiplier so Earth-class O₂ only appears under photosynthetic life), the **chromophore stamp** (`chromophoreGas` + `chromophoreFrac` from `CHROMOPHORE_BY_CLASS` — branched per class with insolation/biosphere gates, gives Earth-like rockies H2O cloud cover, hot sub-Neptunes silicate haze, cold sub-Neptunes Hycean H2O, Titan-class ices CH4 tholin, hot Jupiters silicate, gas giants NH3, etc.), and finally a six-scalar resource grid (`resMetals` / `resSilicates` / `resVolatiles` / `resRareEarths` / `resRadioactives` / `resExotics`) from `PLANET_RESOURCE_PRIORS_BY_CLASS`. Moons traverse the same passes as planets — a moon assigned `worldClass=ice` gets the ice-world atmosphere and resource priors, same as an ice-class planet would.
...
Star discs are drawn by `makeStarMeshMaterial` — a Mesh-path shader that takes `uCenter` (in buffer-pixel coords) and `uRadius` uniforms and discards fragments outside the disc via `length(gl_FragCoord.xy − uCenter) > uRadius`. No parity-aware snap is needed because the discs are large and their centers are CPU-positioned at integer pixels by the layout pass. Planets and moons render through `makePlanetMaterial` — a `gl.POINTS`-based shader that keeps the parity-aware center snap and `gl_FragCoord − vCenter` disc discard from the perspective stars path, but replaces the flat fill with one of two procedural textures driven by per-vertex attributes (`aPalette0..2`, `aWeights`, `aMode`, `aSeed`, `aTilt`, `aWaterFrac`, `aIceFrac`): surface mode (`aMode=0`) runs a worley/voronoi pass over the disc — cells of `SURFACE_PATCH_PX` width with per-body jittered centers — and each fragment picks from the palette by hashing its nearest cell, giving organic lumpy ground cover (Mercury reads as iron-grey + rare-earth rose + silicate rust, mixed by gameplay-grid magnitudes); banded mode (`aMode=1`) treats the disc as the projection of a sphere whose pole is tipped forward by `arcsin(RING_MINOR_OVER_MAJOR)` and whose rotation axis is rotated in-plane by `aTilt` (= `bodyVisualTiltRad(body)`, same helper that drives ring tilt) — bands then trace out arched latitude lines parallel to the ring plane rather than straight horizontal strips. Per-disc band count = `clamp(vRadius × BAND_DENSITY, 6, MAX_BAND_COUNT)` so a Uranus-class disc gets ~30 bands and a Venus-class moon gets ~14, with seed-jittered widths and a chunky horizontal boundary warp so band edges undulate like turbulent zonal flow. Each band picks a palette entry by per-band hash (Jupiter, Titan, Uranus). The per-body palette and mode come from `disc-palette.ts` (see "Procedural disc texture" below). See the "Pixel-perfect rendering" notes below for the shared snap math.

### Procedural disc texture (planets + moons)

The planet/moon disc shader (`makePlanetMaterial`) is fed a fixed-size, 3-entry palette plus a render mode per body. The job of `disc-palette.ts` (`buildDiscPalette`) is to derive those inputs from each body's data — world class, resource grid, atmospheric composition, chromophore, biosphere — so the rendered character of a body emerges from gameplay-relevant fields rather than a separate "appearance" enum.

**Two render modes, one branch per body:**

- **Surface mode** — when `isBandedAtmosphere(body)` returns false. Palette is resource-driven: `[top 3 resources]` by default, or `[top 2 resources, chromophore]` when the body carries a chromophore signal (chromophore at fixed `SURFACE_CHROMOPHORE_WEIGHT = 0.25`; resources split the remaining 0.75 by relative magnitude). World-class color is intentionally absent so the disc reads as gameplay-resource composition rather than rocky/ice/desert taxonomy — Mercury's iron-grey + rare-earth rose + silicate rust ratio mirrors its mining yields directly. World-class color re-enters only as a flat-fill fallback when a body carries no resource signal at all (procgen edge). Surface texture is a worley/voronoi cell pass (`SURFACE_PATCH_PX` controls cell pitch) — the disc is divided into per-body jittered cells and each pixel takes its nearest cell's palette pick, so ground cover reads as organic lumpy patches rather than per-pixel salt-and-pepper noise. On top of the cell layer the shader paints two terrain features driven by the body's primary surface attributes: a sphere-projected polar cap covering latitudes past `1 − iceFraction` (Europa at 1.0 fills the whole disc; Earth at 0.1 leaves a small cap; Venus at 0 has none), and a coarse-cell ocean pass where every `CONTINENT_GROUP × CONTINENT_GROUP` block of worley cells flips to a flat `OCEAN_COLOR` when its hash falls below `waterFraction` (Earth at 0.71 lands ~71% of continent cells under sea level). Cell boundaries inherit the worley jitter for free, so coastlines aren't grid-aligned. Body brightness is intentionally not modulated by a stored albedo scalar — overall reflectance is already emergent from resource composition + water + ice + chromophore aerosols, and stacking a `mix(floor, 1, albedo)` on top double-counted the same signal.
- **Banded mode** — when `isBandedAtmosphere(body)` returns true. Palette comes from `topGases(body)` (top-3 visually-weighted gases, see below), then collapsed toward their weight-proportional mean by `BAND_BLEND_TOWARD_MEAN` before packing. The picker downstream still selects per band by weight, but each entry now sits close to the dominant tone with small variation rather than full-contrast alternation — three blue bands + one white band reads as three light-blue shades, not blue/white stripes. Since `topGases` weights are visual (`frac × potency × boost`, with chromophores up-ranked), the mean reflects what the eye sees, not molar fraction. The shader rotates the disc into a band-aligned frame using `disc.tilt` (= `bodyVisualTiltRad(body)`, derived from `axialTiltDeg` and shared with the ring layer so a ringed giant's bands run parallel to its ring), projects the disc as a sphere viewed from above with the same `RING_MINOR_OVER_MAJOR` foreshortening the ring uses (so bands curve as latitude lines rather than slicing straight across), divides lat-space into a radius-scaled count of non-uniform-width strips, and perturbs band boundaries with a chunky pixel-art warp so edges undulate like turbulent zonal flow. Each band also takes a small ±`BAND_LIGHTNESS_JITTER` additive RGB shift after its palette pick — invisible against a high-contrast Jovian palette, but resolves to readable cream/tan/dark-tan strips when the palette collapses to near-identical hues (e.g. an H2/He gas giant whose chromophore `frac` is tiny enough that the visible-gas mix reduces to two near-transparent species). Three triggers flip a body into banded mode: gas/ice giant class (no surface exists), `surfacePressureBar ≥ ATMOSPHERE_BANDED_PRESSURE_BAR` (Venus-class thick atmosphere of any composition), or haze-forming chromophore (`HAZE_FORMING_CHROMOPHORES = {CH4, SO2}`) plus pressure ≥ `HAZE_BANDED_PRESSURE_BAR` (Titan-class tholin, Io-class sulfur). Water vapor is intentionally not haze-forming — Earth's cloud decks let the surface show.

**Visual weight ≠ molar fraction.** The naive "paint each gas's color proportionally to its atm fraction" reads wrong because H2 and He are essentially transparent while CH4 absorbs strongly per molecule and NH3/SO2 condense into visible aerosols. `topGases` weights each candidate by `frac × potency × boost`:

- `GAS_POTENCY[gas]` — per-molecule absorption strength. H2/He/N2/Ar sit at 0.1 (transparent); CO2/O2/CO at 1.0; H2O/NH3 at 3.0 (cloud-former); CH4 at 6.0 (Uranus/Neptune blue); SO2 at 8.0 (Venus haze).
- `CHROMOPHORE_VISUAL_BOOST` — multiplier (~75) applied to the chromophore candidate before mixing in. Reflects that condensed-phase species punch above their gas-phase fraction; lets Jupiter's 0.026% NH3 win meaningful palette weight against 90% H2/He.
- `GAS_VISIBILITY_FILTER[worldClass]` — gases physically present but buried under a cloud deck. Gas-giant CH4 (under the NH3 ice layer) is dropped on `gas_giant`/`gas_dwarf` so Jupiter doesn't paint blue from CH4 that's invisible from above. Ice giants intentionally don't filter — CH4 IS what you see on Uranus/Neptune.

**Color tables, two layers:**

- `GAS_COLOR[gas]` — the gas's clear-air hue. Used for atm1/2/3 slot lookups.
- `CHROMOPHORE_COLOR[gas]` — the same gas's condensed-phase product hue, used only when the gas enters via the chromophore slot. Different physics, different color: CH4 gas → cyan-blue (Uranus absorption); CH4 chromophore → tholin orange (Titan haze). NH3 gas → cream; NH3 chromophore → NH4SH brown (Jovian belt). The AtmGas vocabulary also includes two aerosol-only species (`SILICATE`, `DUST`) that never appear in `ATMOSPHERE_GASES_BY_CLASS` — they only ever enter via the chromophore path.

`WORLD_CLASS_TINT` applies a small per-class warm/cool shift to every palette entry — `gas_giant` lerps 25% toward amber so Jupiter reads ruddier than Saturn despite sharing dominant-gas composition. Compensates for chromophore chemistry the gas-mix model can't represent (tholins on Jupiter aren't in our vocabulary).

**Procgen chromophore table** (`CHROMOPHORE_BY_CLASS`) is an ordered list of gated branches per worldClass. Gate vocabulary: `insolationAbove` / `insolationBelow` / `biosphereArchetype` / `tierAtLeast`. The Filler walks the list and picks the first matching branch:

- `rocky` + biotic carbon-aqueous at microbial+ tier → H2O (Earth cloud cover)
- `ocean` → H2O (water clouds, always)
- `desert` → DUST (Mars-class haze)
- `ice` → CH4 (Titan tholin — gated by `surfacePressureBar ≥ 0.01` via `chromophoreFor`'s airless check)
- `lava` → SO2 (Io / Venus aerosol)
- `gas_dwarf` hot (S>5) → SILICATE; cold (S<0.3) → H2O (Hycean); else → NH3 (warm Saturn)
- `gas_giant` hot (S>5) → SILICATE; else → NH3
- `ice_giant` → no chromophore (CH4 wins atm slot 1 on potency alone)

The result: ~10 visually distinct atmospheric archetypes spanning the catalog (biotic Earth, ocean world, dust-hazed Mars, Titan, Venus/Io, hot sub-Neptune, Hycean, warm Saturn, cool Jupiter, hot Jupiter, Uranus/Neptune). Curated Sol bodies (Jupiter NH3 / Earth H2O / Saturn NH3 / Titan CH4) set chromophores explicitly in `bodies.csv`; the rest come from procgen.
